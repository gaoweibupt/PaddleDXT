...
## 源码编译和安装
### 环境准备
PaddleDTX使用golang进行开发, 当您使用源码进行编译和安装时, 首先需要准备源码以及编译的环境:
...
4. 编译区块链合约
	...
	```
	编译产出为 paddlempc 文件，为安装在xchain 区块链上的合约文件
...
1. 部署区块链网络
	...
	<br>
	这里我们使用的百度超级链xchain v3.9作为底层区块链网络, 可参考[XuperChain环境部署](https://xuperchain.readthedocs.io/zh/latest/quickstart.html)来搭建区块链网络
	<br>
	您需要了解如何创建合约账户、部署智能合约, 详细参考[部署 native 合约](https://xuperchain.readthedocs.io/zh/latest/advanced_usage/create_contracts.html), 注意这里我们使用的是 golang native 合约，需要修改xchain.yaml支持 native 合约
	<br>
	合约安装过程如下:
	```
	# 定义合约账户和合约名称
...
2. 部署 XuperDB
	...
	XuperDB 包含两种类型的节点服务, 数据持有节点和存储节点, 我们需要分别启动这两种服务。为了演示方便，我们这里两种类型的服务分别启动一个。
	...
	2. 启动数据持有节点
		...
		**注意:** 一般构建 PaddleDTX 网络至少需要两方参与, 即两个数据持有节点, 对应了两个计算任务执行节点, 这里为了说明方便启动一个数据持有节点, 您也可以根据实际需求自行启动多个数据存储节点和数据持有节点
		配置中的privateKey参数为节点的身份，不同privateKey即对应了不同的身份
